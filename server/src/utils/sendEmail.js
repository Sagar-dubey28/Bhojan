...
const sendEmail = async (to, subject, html) => {
  const transporter = nodemailer.createTransport({
    ...
    secure: false,
    auth: {
      user: "apikey", // fixed value (SendGrid ke liye hamesha yehi hota hai)
      pass: process.env.SENDGRID_PASS, // your API key (secret)
    },
  });

  const mailOptions = {
    from: process.env.EMAIL_FROM, // verified sender from SendGrid
    to,
    subject,
    html,
  };
